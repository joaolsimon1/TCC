...
muni_grande_porte <- dados_pontuais %>% filter(`Produto Interno Bruto` > 1000000) %>% select(Codmun6)
...
cluster_assignments1 <- tsclust(df_final3,
         type = "h",
         distance = "dtw",
         centroid = dba,
          window.size = 20L,
...
for (i in 3:10) {
 ...
 plot(cluster_assignments1, cex = .1, main=paste0('K=',i))

  rect.hclust(cluster_assignments1, border ="red", k=i) 
