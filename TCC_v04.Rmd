...
cvis <- as.data.frame(t(validacao_n_clusters))

cvis$Index <- c(3:10)


for (col in names(cvis)[1:ncol(cvis)-1]) {
  p <- ggplot(cvis, aes_string(x="Index", y=col)) +
    geom_line() +
    geom_point() + 
    labs(title=paste(col), x="Clusters", y="Índice") +
    theme_minimal() +
...
for (var in var_num) {
  p <- ggplot(clusters, aes_string(x = "cluster", y = "COVID-19 SMR", fill = "cluster")) +
        geom_boxplot() +
